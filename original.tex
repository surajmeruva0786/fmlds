...
To improve multimodal prediction performance, this study uses a hybrid deep learning system consisting of 3D convolutional neural networks for spatial-temporal facial modeling, EfficientNet-B0 for spatial feature extraction, an Action Unit (AU) gating system, and an audio convolutional branch. The system also relies on uncertainty-aware mechanisms to enhance reliability and deployment safety.

The proposed framework combines Monte Carlo Dropout for predictive uncertainty estimation, temperature scaling for confidence calibration, test-time augmentation for robustness, and selective prediction for exclusion of ambiguous samples. Monte Carlo Dropout approximates Bayesian inference through stochastic forward passes, allowing computation of predictive mean and variance. Temperature scaling corrects overconfident probability estimates without affecting classification accuracy. Selective prediction allows the system to reject high-uncertainty predictions, improving reliability in safety-critical environments. 

The experimental analysis includes a comparative study of baseline CNN-LSTM, transformer-based cross-attention, and the proposed hybrid architecture. Results indicate that while deterministic models achieve high accuracy, incorporating uncertainty estimation improves reliability, decision confidence, and calibration, reducing overconfidence and improving selective performance. This research supports the development of AI-powered stress monitoring systems that produce calibrated probabilistic outputs and account for uncertainty in decision-making. These systems are useful for telemedicine platforms, preventive mental health screening, and occupational stress monitoring. Future studies could investigate real-time deployment with edge-based multimodal sensing systems, conformal prediction frameworks, and out-of-distribution detection. Although multimodal stress detection and uncertainty estimation have been studied separately, their integration for calibrated, selective stress prediction has received limited attention. By integrating selective prediction, probabilistic calibration, and hybrid deep feature learning into a single framework, UA-HybridNet addresses this gap and improves deployment reliability in safety-critical stress-monitoring scenarios.

The main contributions of this work are:

\begin{itemize}
\item An uncertainty-aware multimodal stress detection framework combining 3D-CNN, EfficientNet-B0, and AU gating.
...
Deep learning models often produce overconfident predictions. To improve uncertainty estimation and calibration, Monte Carlo Dropout~\cite{ref9} and temperature scaling~\cite{ref10} are used. Kendall and Gal~\cite{ref11} emphasized capturing both data noise and model uncertainty by distinguishing between aleatoric (random) and epistemic (reducible) uncertainty. Deep ensemble techniques by Lakshminarayanan et al.~\cite{ref12} improve predictive performance using independently trained networks. Geifman and El-Yaniv~\cite{ref13} proposed selective classification to avoid low-confidence predictions. Hendrycks and Gimpel~\cite{ref14} proposed methods for detecting misclassified and out-of-distribution samples. Recent work by Nag et al.~\cite{ref15} shows that optimization strategies can improve stress prediction in occupational settings. Reliable AI systems require clear decision boundaries, calibrated confidence estimates, and accurate predictions. The ability to identify unknown or ambiguous inputs is important in safety-critical applications such as healthcare stress monitoring. These uncertainty-aware techniques support dependable multimodal stress detection frameworks for real-world use.

\subsection{Research Gap}

Despite these advancements, several limitations remain. Many stress detection systems rely on wearable physiological sensors, limiting scalability and practical use. Most existing deep learning techniques produce deterministic outputs without reliable uncertainty estimation. There is a need for a multimodal stress detection system that combines audio-visual data with uncertainty-aware methods.
\section{Stress Prediction Model Overview}

\subsection{Purpose}

For the proposed approach our objective was to provide an accurate multimodal framework for automatic stress detection using facial expression and audio data. Traditional unimodal systems for stress detection often perform poorly because they rely solely on visual or auditory data and lack contextual cues. To overcome this limitation, the proposed approach combines speech-based acoustic representations with face video data using a hybrid deep learning architecture. The system uses EfficientNet-B0 to extract spatial representations from facial frames and a 3D Convolutional Neural Network (3D-CNN) to capture spatial–temporal facial dynamics. EfficientNet architectures have achieved strong performance in visual recognition challenges~\cite{ref6}. The framework aims to enhance stress recognition performance and provide reliable classification of stress levels by combining visual and auditory modalities.
...
Figure~\ref{fig:sample} illustrates the proposed multimodal stress detection framework. Data collection, preprocessing, feature extraction, multimodal fusion, and classification are part of the system pipeline. The RAVDESS dataset is the primary data source, containing emotional speech as audio-video recordings. The dataset includes facial video frames and voice signals~\cite{ref5}. Preprocessing techniques such as frame extraction, face detection, audio normalization, and temporal segmentation are used to ensure input quality. For visual analysis, a hybrid architecture combining 3D-CNN and EfficientNet-B0 captures temporal dynamics and spatial facial representations. Audio characteristics associated with stress are captured by extracting acoustic features from speech data. A multimodal feature fusion approach combines visual and auditory data. Fully connected layers are used for final classification. Two categories: \textit{Stress} and \textit{Non-Stress} are predicted by the output layer. Multimodal learning techniques improve robustness and classification performance~\cite{ref2}.

\subsection{Dataset}

The publicly available RAVDESS dataset is used in this study. It contains audio–visual recordings of speeches with different emotional expressions performed by 24 professional actors (12 male and 12 female)~\cite{ref5}. Eight emotional states—neutral, calm, happy, sad, angry, fearful, disgusted, and surprised—are expressed by each actor. The recordings include high-quality video synchronized with speech signals. For stress detection, emotions like anger, fear, and sadness are categorized as stress-related, while calm and neutral are considered non-stress. RAVDESS contains controlled, synchronized audio and video recordings suitable for testing multimodal learning architectures. The structured nature of the dataset enables evaluation of calibration and uncertainty mechanisms. Future work will expand UA-HybridNet to real-world stress datasets and in-the-wild recordings.

\subsection{Data Preprocessing}

Preprocessing enhances model performance and reliability. The pipeline includes facial frame extraction, face detection, image resizing, and normalization. Video clips are converted into frame sequences, and facial regions are detected and extracted. Images are resized to a consistent resolution for deep learning models. For audio, speech signals are normalized and converted into Mel-spectrogram representations, capturing important acoustic features of stress and speech dynamics.

\subsection{Data Segmentation}

Temporal segmentation divides video data into smaller sequences to capture motion. Each video is split into fixed-length frame sequences, enabling the 3D-CNN model to capture temporal changes in facial expressions.
Similarly, audio signals are divided into short intervals to generate spectrogram representations that capture speech dynamics related to stress.
\subsection{Feature Extraction Using Deep Learning}

    Deep learning architectures that automatically learns selective features on visual and acoustic modalities.

\begin{itemize}

\item \textbf{3D Convolutional Neural Network (3D-CNN)}

The 3D-CNN model extracts spatio-temporal features from facial video sequences. In contrast to other 2D CNNs, 3D CNNs combine spatial and temporal dimensions of convolution, which allows the model to learn patterns of facial expressions~\cite{ref7}.
...
From the facial frame the high-level spatial features are extracted using the EfficientNet-B0. EfficientNet employs compound scaling to balance network depth, width, and input resolution, enabling efficient feature learning with fewer parameters~\cite{ref6}.
\begin{equation}
...
In order to use the complementary information provided by the speech and facial modalities, we have used Feature-level fusion. The visual and audio feature vectors are obtained and merged together to have a unified representation. 
\begin{equation}
...
To utilise its multimodal representation, the model is capable of obtaining dynamics of facial expressions and speech characteristics that are concerned with stress. Multimodal fusion techniques have found wide application in affective computing to enhance the robustness and reliability of prediction ~\cite{ref2}.
\subsection{Model Training and Stress Classification}

To classify the fused feature vector, we have applied fully connected layers. The final prediction layer predicts the probability distribution of stress classes using the SoftMax activation function.
\begin{equation}
P(y=k|x) = \frac{e^{z_k}}{\sum_{j=1}^{K} e^{z_j}}
\end{equation}

Two classification, \textit{Stress} and \textit{Non-Stress}, corresponds to the final output as a task of binary classification. During training, to optimize the model's parameters we have used backpropagation. Standard metrics, such as accuracy, precision, recall, F1-score, and Area Under the Curve (AUC), are used to evaluate performance and are also frequently used to assess the stress detection systems in machine learning research.

\section{Results and Discussion}

This section provides a comprehensive analysis of \textbf{UA-HybridNet}, which combines uncertainty estimation and probability calibration with the Hybrid (3D-CNN + EfficientNet-B0 + AU) architecture.
...
The deterministic Hybrid model includes probabilistic calibration and uncertainty estimates, which significantly improves the dependability and interpretability of predictions, even though the achieved raw accuracy is slightly higher. So as a result, the proposed methodology places more importance on reliable predictions than just focusing on point-estimate accuracy.
...
Hybrid (3D-CNN + EfficientNet-B0 + Aux.)
...
For a balanced test set of 150 samples (75 stress and 75 non-stress) the per-class classification report is shown in Table~\ref{tab:perclass_pipelineD}. While maintaining balanced macro and weighted averages across precision, recall, and F1-score, for the non-stress class, the results show slightly better detection performance.
\begin{table}[htpb]
\centering
\caption{TABLE III: Per-Class Classification Report for UA-HybridNet}
...
The risk-coverage curve in Fig.~\ref{fig:risk_curve} shows that rejecting high-uncertainty samples significantly reduces prediction error.
\begin{figure}[H]
\centering
...
Under uncertainty constraints the Table~\ref{tab:clinical_safety} evaluates the stress recall. Excluding the uncertain samples improves the recall for stress detection, an important factor in the applications of safety-sensitive monitoring.
...
Next, the size of the dataset is quite modest compared with other large-scale deep learning datasets. Larger multimodal datasets and real-time deployment situations will be the focus of future research.

\section{Conclusion}


To detect facial stress, this research proposed an uncertainty-aware deep learning system which combines probabilistic calibration and uncertainty estimation with hybrid feature learning. The proposed model involves Action Unit gating with the Monte-Carlo Dropout and temperature scaling, EfficientNet-B0 spatial feature extraction, and 3D-CNN spatio-temporal encoding. A very strong classification performance is shown by the experimental findings on the RAVDESS dataset, with an accuracy of 0.8133 and an AUC of 0.8782. By reducing the negative log-likelihood by 45.1\% and the Expected Calibration Error (ECE) from 0.1768 to 0.0710, the calibration is greatly improved by temperature scaling and showed an increased agreement between actual accuracy and expected confidence.
